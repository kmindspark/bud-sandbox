...
class Client
  ...
  bloom do
      ...
      message_func_allreduce <~ (ips_allreduce * data_allreduce * timer).pairs {|i, d, t| [i.val, d.key, d.val] if hello() and i.key == d.key}

      #stdio <~ received_allreduce.inspected

      message_func_allreduce <~ (message_func_allreduce * data_allreduce * ips_allreduce).combos \
          {|m, d, i| [i.val, m.key, d.val + m.val] if i.key == (m.key + 1) % 4 and m.key == d.key and d.key != @client_id}

      send_around <~ (message_func_allreduce * ips_allreduce).combos \
          {|m, i| [i.val, m.key, m.val] if i.key == (m.key + 1) % 4 and m.key == @client_id}

      send_around <~ (send_around * ips_allreduce).combos \
          {|s, i| [i.val, s.key, s.val] if i.key == (m.key + 1) % 4 and d.key != @client_id}
